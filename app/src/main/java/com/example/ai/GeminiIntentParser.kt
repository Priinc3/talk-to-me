...
data class ParsedActionsResult(
    val transcript: String,
    val summaryText: String,
    val spokenConfirmation: String,
    val todosToCreate: List<TodoEntity> = emptyList(),
    val calendarBlocksToCreate: List<CalendarBlockEntity> = emptyList(),
    val remindersToCreate: List<ReminderEntity> = emptyList(),
    val alarmsToCreate: List<VoiceAlarmEntity> = emptyList(),
    val meetingNotesToCreate: List<MeetingNoteEntity> = emptyList()
)
...
class GeminiIntentParser {
    ...
    suspend fun parseVoiceCommand(transcript: String): ParsedActionsResult = withContext(Dispatchers.IO) {
        ...
        if (apiKey.isNotBlank() && !apiKey.contains("MY_GEMINI_API_KEY")) {
            try {
                return@withContext callGeminiApi(transcript, apiKey)
            } catch (e: Exception) {
                e.printStackTrace()
            }
        }
    ...
    private fun callGeminiApi(transcript: String, apiKey: String): ParsedActionsResult {
        ...
        val systemPrompt = """
            ...
            JSON schema format:
            {
              ...
              "todos": [
                { "text": "Task description", "dueDate": "Optional date string" }
              ],
              "calendarBlocks": [
                { "title": "Meeting Title", "startTimeMillis": 123456789, "durationMinutes": 30, "location": "" }
              ],
              "reminders": [
            ...
            put("contents", JSONArray().apply {
                put(JSONObject().apply {
                    put("parts", JSONArray().apply {
                        put(JSONObject().put("text", "Parse this user voice command: \"$transcript\""))
                    })
                })
            })
        ...
        val text = content?.optJSONArray("parts")?.optJSONObject(0)?.optString("text")
            ?: throw Exception("No text in candidate")
    ...
    private fun parseWithLocalRuleEngine(transcript: String): ParsedActionsResult {
        ...
        when {
            // Case 1: "meeting in half an hour" or "meeting in 30 minutes"
            ...
            // Case 2: Alarm request e.g. "set alarm" or "alarm for" or "alarm in"
            lower.contains("alarm") -> {
                val alarmTime = now + 60 * 60 * 1000L // 1 hour default
            ...
            // Case 3: Reminder e.g. "remind me to..."
            lower.contains("remind me") || lower.contains("reminder") -> {
                ...
                val remTime = now + 15 * 60 * 1000L // 15 mins
            ...
            // Case 4: Meeting note e.g. "record note" or "meeting note"
            ...
            // Default Case: Add as Todo item + Calendar Block if date mentioned
            else -> {
